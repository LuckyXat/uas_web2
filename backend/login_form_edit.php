...
<?php  
	...
	$edit = mysqli_query($conn,"select * from admin where admin_ID = '$kodeadmin'");
	...
	if (isset($_POST['ubah'])) {
		...
		mysqli_query($conn, "UPDATE admin SET admin_USER='$adminUSER', admin_PASS='$adminPASS' where admin_ID='$kodeadmin'");  //ganti $adminPASShash untuk jadi password hash
...
<body>
	...
	<form method="POST">
		...
		<div class="row mb-3 mt-5">
			...
			<div class="col-sm-10">
				<input type="text" class="form-control" id="adminID" name="inputID" value="<?php echo $row_edit["admin_ID"] ?>">
			</div>
		</div>
		<div class="row mb-3">
			<label for="adminUSER" class="col-sm-2 col-form-label">Username Admin</label>
			<div class="col-sm-10">
				<input type="text" class="form-control" id="adminUSER" name="inputUSER" value="<?php echo $row_edit["admin_USER"] ?>">
			</div>
		</div>
		<div class="row mb-3">
			<label for="adminPASS" class="col-sm-2 col-form-label">Password Admin</label>
			<div class="col-sm-10">
				<input type="password" class="form-control" id="adminPASS" name="inputPASS">
			</div>
		</div>
